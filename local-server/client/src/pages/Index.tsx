  ...
  const parseCSVToResults = (csvText: string): ReceiptData[] => {
    ...
    return dataRows.map(row => {
      ...
      headers.forEach((header, index) => {
        obj[header.trim()] = values[index]?.replace(/^"|"$/g, '').replace(/""/g, '"').trim() || '';
      });
      ...
      <header className="border-b bg-card">
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Receipt Scanner (Local)</h1>
              <p className="text-sm text-muted-foreground">
                AI-powered batch processing for up to 5 receipts at once
              </p>
            </div>
          </div>
        </div>
      ...
      <footer className="mt-24 border-t bg-card">
        <div className="container mx-auto px-4 py-8">
          <p className="text-center text-sm text-muted-foreground">
            Powered by Gemini 2.0 • Local Processing • Extracts receipts in any language and currency
          </p>
        </div>
      </footer>
    </div>
